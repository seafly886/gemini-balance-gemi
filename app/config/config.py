...
class Settings(BaseSettings):
    ...
    # github
    GITHUB_REPO_OWNER: str = "seafly886"
    ...
    # Key Usage Mode Configuration
    KEY_USAGE_MODE: str = Field(
        default="polling",
        description="Key usage mode: 'polling' for round-robin, 'fixed' for fixed key with threshold switching"
    )
    KEY_USAGE_THRESHOLD: int = Field(
        default=200,
        ge=1,
        le=10000,
        description="Usage threshold for fixed key mode before switching to next key"
    )
