...
async function fetchAndDisplayKeys(type, page = 1) {
    ...
    try {
        ...
        if (Object.keys(keys).length > 0) {
            Object.entries(keys).forEach(([key, keyInfo]) => {
                // Handle both old format (number) and new format (object)
                let fail_count, usage_count;
                if (typeof keyInfo === 'object' && keyInfo !== null) {
                    fail_count = keyInfo.fail_count || 0;
                    usage_count = keyInfo.usage_count || 0;
                } else {
                    fail_count = keyInfo || 0;
                    usage_count = 0;
                }
                const listItem = createKeyListItem(key, fail_count, usage_count, type);
 ...
 * @param {number} usage_count The usage count for the key.
 * @param {string} type 'valid' or 'invalid'.
 * @returns {HTMLElement} The created list item element.
 */
function createKeyListItem(key, fail_count, usage_count, type) {
    const li = document.createElement("li");
    ...
    li.dataset.usageCount = usage_count;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initializeKeyUsageMode(); // 初始化Key使用模式功能
...
// 全局变量存储当前模式状态
let currentKeyModeStatus = null;

// 初始化Key使用模式功能
async function initializeKeyUsageMode() {
  try {
    await refreshKeyModeStatus();
    // 设置定时刷新状态（每30秒）
    setInterval(refreshKeyModeStatus, 30000);
  } catch (error) {
    console.error("Failed to initialize key usage mode:", error);
    showNotification("初始化Key使用模式失败", "error");
  }
}

// 刷新Key模式状态
async function refreshKeyModeStatus() {
  try {
    const response = await fetchAPI('/api/keys/usage-mode');
    currentKeyModeStatus = response;
    updateKeyModeUI(response);
  } catch (error) {
    console.error("Failed to refresh key mode status:", error);
    showNotification("刷新Key模式状态失败", "error");
  }
}

// 更新Key模式UI显示
function updateKeyModeUI(status) {
  const modeToggle = document.getElementById('keyUsageModeToggle');
  const modeDescription = document.getElementById('modeDescription');
  const keyModeStatus = document.getElementById('keyModeStatus');
  ...
  const usageThreshold = document.getElementById('usageThreshold');
  const currentKeyDisplay = document.getElementById('currentKeyDisplay');
  const currentKeyUsage = document.getElementById('currentKeyUsage');
  const currentThreshold = document.getElementById('currentThreshold');

  if (!status) return;

  // 更新模式开关
  const isFixedMode = status.usage_mode === 'fixed';
  modeToggle.checked = isFixedMode;

  // 更新模式描述
  if (isFixedMode) {
    modeDescription.textContent = '固定模式：使用单个key直到达到阈值';
    thresholdSettings.classList.remove('hidden');
    currentKeyStatus.classList.remove('hidden');
  } else {
    modeDescription.textContent = '轮询模式：依次使用每个key';
    thresholdSettings.classList.add('hidden');
    currentKeyStatus.classList.add('hidden');
  }

  // 更新状态显示
  keyModeStatus.textContent = `当前: ${isFixedMode ? '固定模式' : '轮询模式'}`;

  // 更新阈值设置
  if (usageThreshold) {
    usageThreshold.value = status.usage_threshold || 200;
  }

  // 更新当前key状态（仅在固定模式下）
  if (isFixedMode && status.current_fixed_key) {
    const keyDisplay = status.current_fixed_key.substring(0, 4) + '...' +
                     status.current_fixed_key.substring(status.current_fixed_key.length - 4);
    currentKeyDisplay.textContent = keyDisplay;
    currentKeyUsage.textContent = status.current_key_usage || 0;
    currentThreshold.textContent = status.usage_threshold || 200;
  }
}

// 切换Key使用模式
async function toggleKeyUsageMode() {
  const modeToggle = document.getElementById('keyUsageModeToggle');
  const newMode = modeToggle.checked ? 'fixed' : 'polling';
  ...
  try {
    ...
    // 如果切换到固定模式，包含当前阈值
    if (newMode === 'fixed') {
      const thresholdInput = document.getElementById('usageThreshold');
      if (thresholdInput) {
        requestData.threshold = parseInt(thresholdInput.value, 10) || 200;
      }
    }
    ...
    if (response.success) {
      showNotification(`已切换到${newMode === 'fixed' ? '固定' : '轮询'}模式`, "success");
      // 更新UI状态
      updateKeyModeUI(response.status);
      currentKeyModeStatus = response.status;
    } else {
      throw new Error(response.message || '切换模式失败');
    }
  } catch (error) {
    console.error("Failed to toggle key usage mode:", error);
    showNotification(`切换模式失败: ${error.message}`, "error");
    // 恢复开关状态
...
// 更新使用阈值
async function updateUsageThreshold() {
  const thresholdInput = document.getElementById('usageThreshold');
  const threshold = parseInt(thresholdInput.value, 10);

  if (!threshold || threshold < 1) {
    showNotification("阈值必须大于0", "warning");
    thresholdInput.value = currentKeyModeStatus?.usage_threshold || 200;
    return;
  }
  ...
  try {
    ...
    if (response.success) {
      showNotification(`阈值已更新为${threshold}次`, "success");
      updateKeyModeUI(response.status);
      currentKeyModeStatus = response.status;
    } else {
      throw new Error(response.message || '更新阈值失败');
    }
  } catch (error) {
    console.error("Failed to update usage threshold:", error);
    showNotification(`更新阈值失败: ${error.message}`, "error");
    // 恢复原值
    thresholdInput.value = currentKeyModeStatus?.usage_threshold || 200;
  }
}
...
async function resetUsageCounts() {
  const resetBtn = document.getElementById('resetUsageCountsBtn');

  try {
    resetBtn.disabled = true;
    const originalHtml = resetBtn.innerHTML;
    resetBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-1"></i>重置中...';

    const response = await fetchAPI('/api/keys/reset-usage-counts', {
      method: 'POST'
    });

    if (response.success) {
      showNotification("使用计数已重置", "success");
      // 刷新状态
      await refreshKeyModeStatus();
    } else {
    ...
    showNotification(`重置使用计数失败: ${error.message}`, "error");
  } finally {
    resetBtn.disabled = false;
    resetBtn.innerHTML = '<i class="fas fa-redo-alt mr-1"></i>重置计数';
  }
}
